...
export default function BalanceChart({ transactions, isStealthMode }) {
  ...
  transactions.forEach(tx => {
    ...
    const category = tx.category || '不明'; // 振替先
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '25px', height: '100%' }}>
      ...
      <div style={{ background: '#11141a', padding: '20px', borderRadius: '8px', border: '1px solid #252838', flex: 1 }}>
        ...
        {/* CSS Grid でカートリッジを美しく並べる！ */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
          {sortedBalances.map(([name, amount]) => {
            // その口座が総資産の何%を占めるか計算（ゲージ用）
            ...
            return (
              ...
              >
                ...
                <div style={{ color: isNegative ? '#ff3366' : '#fff', fontSize: '24px', fontWeight: 'bold', fontFamily: 'monospace', textAlign: 'right' }}>
                  {isStealthMode ? '¥***' : `¥${amount.toLocaleString()}`}
                </div>

                {/* 🌟 発光エネルギーゲージ（アニメーション） */}
                <div style={{ 
                  position: 'absolute', bottom: 0, left: 0, height: '4px', width: '100%', background: '#1a1d24' 
                }}>
